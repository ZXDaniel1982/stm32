...
static void LCD_Clear(void)
{
	...
	LCD_WR_CMD(0x04, (uint8_t)(239 >> 8));
	LCD_WR_CMD(0x05, (uint8_t)239);	//Column End

	LCD_WR_CMD(0x06, 0);
	LCD_WR_CMD(0x07, 0);	//Row Start
	LCD_WR_CMD(0x08, (uint8_t)(319 >> 8));
	LCD_WR_CMD(0x09, (uint8_t)319);	//Row End

	LCD_WR_REG(34);
	temp = (uint32_t) 240 * 320;
...
void LCD_Init(void)
{
  ...
  LCD_WR_CMD(0x24,0x2f); //VMH(VCOM High voltage ~4.2V)
  LCD_WR_CMD(0x25,0x57); //VML(VCOM Low voltage -1.2V)
  //****VCOM offset**///
  LCD_WR_CMD(0x23,0x88); //for Flicker adjust //can reload from OTP
  //Power on Setting
  LCD_WR_CMD(0x18,0x34); //I/P_RADJ,N/P_RADJ, Normal mode 70Hz
  LCD_WR_CMD(0x19,0x01); //OSC_EN='1', start Osc
  LCD_WR_CMD(0x01,0x00); //DP_STB='0', out deep sleep
  LCD_WR_CMD(0x1F,0x88);// GAS=1, VOMG=10, PON=1, DK=0, XDK=0, DDVDH_TRI=0, STB=0
  for (i=0;i<72000;++i) {}
  LCD_WR_CMD(0x1F,0x80);// GAS=1, VOMG=10, PON=1, DK=0, XDK=0, DDVDH_TRI=0, STB=0
  for (i=0;i<72000;++i) {}
  LCD_WR_CMD(0x1F,0x90);// GAS=1, VOMG=10, PON=1, DK=0, XDK=0, DDVDH_TRI=0, STB=0
  for (i=0;i<72000;++i) {}
  ...
  //SET PANEL
  LCD_WR_CMD(0x36,0x00); //SS_P, GS_P,REV_P,BGR_P
  LCD_WR_CMD(0x28,0x38); //GON=1, DTE=1, D=1100
  for (i=0;i<72000;++i) {}
...
static void TFT_ShowChar(uint8_t x, uint16_t y, uint8_t num)
{
	...
	LCD_WR_CMD(0x04, (uint8_t)((x + 5) >> 8));
	LCD_WR_CMD(0x05, (uint8_t)(x + 5));	//Column End

	LCD_WR_CMD(0x06, (uint8_t)(y >> 8));
	LCD_WR_CMD(0x07, (uint8_t)y);	//Row Start
	LCD_WR_CMD(0x08, (uint8_t)((y + 11) >> 8));
