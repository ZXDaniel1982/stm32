...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_ReadBlock(uint32_t * readbuff, uint32_t ReadAddr,
					  uint16_t BlockSize)
{
	SD_Error errorstatus = SD_OK;
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_WriteBlock(uint32_t * writebuff, uint32_t WriteAddr,
					   uint16_t BlockSize)
{
	SD_Error errorstatus = SD_OK;
			...
			if ((TotalNumberOfBytes - bytestransferred) < 32) {
				restwords =
					((TotalNumberOfBytes - bytestransferred) % 4 ==
					 0) ? ((TotalNumberOfBytes -
							bytestransferred) / 4) : ((TotalNumberOfBytes -
													   bytestransferred) / 4 +
													  1);
	...
	if (DeviceMode == SD_INTERRUPT_MODE) {
		if (SDIO_GetITStatus(SDIO_IT_RXFIFOHF) != RESET) {
			...
			if ((TotalNumberOfBytes - NumberOfBytes) < SD_HALFFIFOBYTES) {
				...
				for (count = 0; count < restwords;
					 count++, SrcBuffer++, NumberOfBytes += 4) {
