...
{
    uint32_t i;
    MODIFY_REG(GPIOE->ODR, GPIO_BSRR_BS1, 0);
    for (i=0;i<72000;++i) {}
    MODIFY_REG(GPIOE->ODR, GPIO_BSRR_BS1, GPIO_BSRR_BS1);
    for (i=0;i<72000;++i) {}

    MODIFY_REG(GPIOD->ODR, GPIO_BSRR_BS13, GPIO_BSRR_BS13);
}
...
void
HX8347D::LCD_WR_CMD(unsigned int index,unsigned int val)
{
...
void
HX8347D::LCD_WR_Data(unsigned int val)
{
    *(__IO uint16_t *) (Bank1_LCD_D)= val;
}

void
HX8347D::LCD_WR_REG(unsigned int index)
{
    *(__IO uint16_t *) (Bank1_LCD_C)= index;
}

void
HX8347D::TFT_ShowChar(uint8_t x,uint16_t y,uint8_t num)
{
    uint8_t temp;
    uint8_t pos,t;
    ...
    for(pos=0;pos<12;pos++) {
        ...
        for(t=0;t<6;t++)
        {
            if(temp&0x01)LCD_WR_Data(POINT_COLOR);    //??1 ??????????
            else LCD_WR_Data(0xffff);                 //??0 ????????
            temp>>=1;
        }
    }
}
...
void
HX8347D::TFT_ShowString(uint8_t x,uint16_t y,const uint8_t *p)
{
    while(*p!='\0') {
        TFT_ShowChar(x,y,*p);
        x+=6;
