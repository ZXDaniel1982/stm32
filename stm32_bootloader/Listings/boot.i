...
#line 5 "Core\\Src\\boot.c"
#line 6 "Core\\Src\\boot.c"
...
static void boot_saveenv(uint8_t *command)
{
    ...
    EEPROM_Write((0), (uint8_t *) &Bootargs, sizeof(Bootargs_t));
    return;
}
...
static BootState_t GetImageInfo(Imageargs_t *iargs, uint8_t *src)
{
    ...
    if (strcmp((char *)src, "EEPROM") == 0) { 
        EEPROM_Read(((4096)), (uint8_t *) iargs, sizeof(Imageargs_t));
        if (((0x2233) == iargs->header) &&
            ((0x8899) == iargs->tail)) {
        ...
        memset(ImageBuf, 0, (512));
        if (SD_ReadBlock((uint32_t *)ImageBuf, (0), (512)) == SD_OK) {
...
{
    Imageargs_t iargs;
    uint32_t eepAddr = (((4096)) + (4096));
    uint32_t flashAddr = 0x8005000;
    uint32_t size;
    ...
    if (GetImageInfo(&iargs, "EEPROM") == STM32_SUCCESS) {
        ...
        while (size > 0) {
            if ((flashAddr % (0x800U)) == 0) {
                FLASH_Erase(flashAddr);
            }

            memset(ImageBuf, 0xff, (512));
            EEPROM_Read(eepAddr, ImageBuf, (512));
            FLASH_Program((uint8_t *)flashAddr, ImageBuf, (512));
...
static void ImageCopySdToFlash()
{
    ...
    if (GetImageInfo(&iargs, "SD") == STM32_SUCCESS) {
        ...
        while (size > 0) {
            ...
            memset(ImageBuf, 0xff, (512));
            SD_ReadBlock((uint32_t *)ImageBuf, sdAddr, (512));
            FLASH_Program((uint8_t *)flashAddr, ImageBuf, (512));
...
static void ImageCopySdToEep()
{
    ...
    if (GetImageInfo(&iargs, "SD") == STM32_SUCCESS) {
        EEProm_SectorErrase(((4096)));
        EEPROM_Write(((4096)), (uint8_t *) &iargs, sizeof(Imageargs_t));
        ...
        while (size > 0) {
            ...
            memset(ImageBuf, 0xff, (512));
            SD_ReadBlock((uint32_t *)ImageBuf, sdAddr, (512));
            EEPROM_Write(eepAddr, (uint8_t *) ImageBuf, (512));
...
{
    Bootargs.header = (0x4455);
    Bootargs.tail = (0x5566);
...
{
    EEPROM_Read((0), (uint8_t *) &Bootargs, sizeof(Bootargs_t));

    if (((0x4455) == Bootargs.header) &&
        ((0x5566) == Bootargs.tail)) {
        return STM32_SUCCESS;
    } else {
        BootargsInit();
        return STM32_FALSE;
...
void bootm(uint8_t *command)
{
    ...
    if (strcmp((char *)Bootargs.confSrc, (char *)Bootargs.statSrc) == 0) {
        ...
        if (GetImageInfo(&iargs, (uint8_t *)Bootargs.confSrc) == STM32_SUCCESS) {
            memset((char *)Bootargs.statSrc, 0, (10));
...
void AppJump()
{
	  uartprintf("BootAddrData %d\r\n", *(volatile uint32_t *) 0x8005000);
    if (((*(volatile uint32_t *) 0x8005000) & 0x2FFE0000) == 0x20000000) {
