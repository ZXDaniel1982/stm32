...
static void SpcData_CorrectLowTemp(SpcTempGroupConfig_t *tempgroup)
{
    ...
    if ((lowtemp->status == Opt) &&
        ...
        if (lowtemp->temperature[unit] >= hightemp->temperature[unit]) {
            hightemp->temperature[unit] = lowtemp->temperature[unit] + 1;
        }
    }
}
...
static void Page_Config_LowTemp(Logger logger, PageEntity_t *page)
{
    ...
    SpcData_CorrectLowTemp(tempgroup);
...
PageEntity_t *Page_Func_LowTemp(KeyEnum_t key, Logger logger, PageEntity_t *page)
{
    ...
    case Right:
    case Left:
        return Page_CreatePage(Spc_GetNextPage(logger, key, LowTemp), logger, page->publisher);
    case Up:
    case Down:
